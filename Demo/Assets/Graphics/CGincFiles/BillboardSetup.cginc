...
void Get4VectorsUp(float3 center, fixed pixelSize, out float4 vectors[4])
{
	...
	fixed3 right = mul((float3x3)unity_WorldToObject,GetRightVector(look) * pixelSize);
	fixed3 up = mul((float3x3)unity_WorldToObject,fixed3(0, pixelSize * y_scale, 0));
...
void Get4Vectors(float3 center, fixed pixelSize, out float4 vectors[4])
{
	...
	fixed3 look = GetLookVector();
	fixed3 right = mul((float3x3)unity_WorldToObject,GetRightVector(look) * pixelSize);
	fixed3 up = mul((float3x3)unity_WorldToObject,fixed3(0, pixelSize, 0));

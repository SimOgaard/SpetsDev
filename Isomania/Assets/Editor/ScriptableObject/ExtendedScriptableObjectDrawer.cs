...
public class ExtendedScriptableObjectDrawer : PropertyDrawer
{
	...
	public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
	{
		...
		*/
		...
		if (property.propertyType == SerializedPropertyType.ObjectReference && property.objectReferenceValue != null)
		{
			...
			if (GUI.Button(buttonRect, "Varying"))
			{
				// create a varying of current property
				string templateAssetFullPath = AssetDatabase.GetAssetPath(property.objectReferenceValue);
				ScriptableObject serializedProperty = CreateVaryingAssetWithSavePrompt(type, templateAssetFullPath);
				if (serializedProperty != null)
                {
					property.objectReferenceValue = serializedProperty;
				}
			}
		}
		else
		{
			if (GUI.Button(buttonRect, "Create"))
			{
				...
				{
					var guid_path = AssetDatabase.GUIDToAssetPath(guid);
					var guid_type = AssetDatabase.LoadAssetAtPath(guid_path, typeof(Object));
					...
					{
						if (t == type && t.Name == guid_type.name)
						{
							typeFound = true;
							break;
						}
					...
					string selectedAssetPath = System.IO.Path.GetDirectoryName(AssetDatabase.GetAssetPath(guid_type));
	...
	// Creates a new ScriptableObject via the default Save File panel with the same value as current
	static ScriptableObject CreateVaryingAssetWithSavePrompt(Type type, string templateAssetFullPath)
	{
		string templateAssetPath = System.IO.Path.GetDirectoryName(templateAssetFullPath);
		string templateAssetName = System.IO.Path.GetFileNameWithoutExtension(templateAssetFullPath);
		string varyingPath = EditorUtility.SaveFilePanelInProject("Copy ScriptableObject as Varying", templateAssetName + "Varying.asset", "asset", "Enter a file name for the ScriptableObject.", templateAssetPath);
		if (varyingPath == "") return null;
		AssetDatabase.CopyAsset(templateAssetFullPath, varyingPath);
		AssetDatabase.SaveAssets();
		AssetDatabase.Refresh();
		ScriptableObject asset = AssetDatabase.LoadAssetAtPath<ScriptableObject>(varyingPath);
		AssetDatabase.ImportAsset(varyingPath, ImportAssetOptions.ForceUpdate);
		EditorGUIUtility.PingObject(asset);
		return asset;
	}

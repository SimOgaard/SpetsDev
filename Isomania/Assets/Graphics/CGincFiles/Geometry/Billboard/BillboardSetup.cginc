...
void Get4VectorsUp(float3 center, float pixelSize, out float4 vectors[4])
{
	...
	vectors[0] = mul(unity_WorldToObject, float4(center + right - up, 1.0f));
	vectors[1] = mul(unity_WorldToObject, float4(center + right + up, 1.0f));
	vectors[2] = mul(unity_WorldToObject, float4(center - right - up, 1.0f));
	vectors[3] = mul(unity_WorldToObject, float4(center - right + up, 1.0f));
}

#include "/Assets/Graphics/CGincFiles/Billboard/BillboardSetup.cginc"
...
struct g2f
{
	...
	float2 toonUV : TEXCOORD0;
	float2 wind : TEXCOORD1;
	float2 uv : TEXCOORD2;
};

g2f VertexOutput(float3 pos, float2 toonUV, float2 wind)
{
	g2f o;
...
void geo(triangle vertexOutput IN[3], inout TriangleStream<g2f> outStream)
{
	...
	float2 toonUV = CalculateToonUVFlat(world_center, flatNormal);
	...
	g2f idealVertexOutput = VertexOutput(center, toonUV, wind);
